...
def plot_length_his(input_texts, f):
...
for i, doc in enumerate(input_texts):
    ...
    input_sentences.append(sent_tokenize_list)
    for s in sent_tokenize_list:
        all_sentences.append(s)
# sentence length distribution
plot_length_his(all_sentences, 'sentencelength.jpg')
...
to_remove = set(only_once + only_twice + most_common_100)
...
plot_length_his(input_texts, 'doclength.jpg')
...
counter = 0
not_found_list = []
embeddings = np.random.uniform(-.3, .3, size=(voc_len+1, 200))
...
input_texts_embedding = np.zeros((len(input_texts), l_max, 200))
...
rnn = BiGRU(input_size, hidden_size, num_layers, num_classes)
