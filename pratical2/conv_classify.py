...
# A convolutional classifier for TED talks text cat
#download dataset
...
class CNN_Text(nn.Module):
    def __init__(self, input_size, num_classes):
        ...
        self.conv2d2 = torch.nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(2, input_size))
        self.conv2d3 = torch.nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(3, input_size))
        self.conv2d4 = torch.nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(4, input_size))
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        o2 = self.conv2d2(x)
        o2 = self.relu(o2)
        o2 = torch.squeeze(o2, 1)
        o2 = torch.max(o2, 1)[0]
        o3 = self.conv2d3(x)
        o3 = self.relu(o3)
        o3 = torch.squeeze(o3, 1)
        o3 = torch.max(o3, 1)[0]
        o4 = self.conv2d4(x)
        o4 = self.relu(o4)
        o4 = torch.squeeze(o4, 1)
        o4 = torch.max(o4, 1)[0]
        out = torch.cat((o2, o3, o4), 1)
        out = self.fc(out)
        return out

net = CNN_Text(input_size, num_classes)
